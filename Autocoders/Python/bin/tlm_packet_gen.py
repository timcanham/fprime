...
#            return None
    ...
    def gen_packet_file(self, xml_filename):
        ...
        if not "PacketsAi" in xml_filename:
            raise IOError("ERROR: Missing PacketsAi at end of file name %s"%xml_filename)
            ...
            for entry in element_tree.getroot():
                ...
                if entry.tag == "import_topology":
                    ...
                    if packet_id in id_list:
                        raise TlmPacketParseValueError("Duplicate packet id %s"%packet_id)
                    else:
                    ...
                    for channel in entry:
                        ...
                        if not channel_name in channel_size_dict:
                            raise TlmPacketParseValueError("Channel %s does not exist"%channel_name)
                        (channel_id, channel_size) = channel_size_dict[channel_name]
                        packet_size += channel_size
                        if self.verbose:
                            print(" -Channel %s ID %d size %d"%(channel_name,channel_id,channel_size))
                        channel_list.append((channel_id,channel_size,channel_name))
                        packetized_channel_list.append(channel_name)
                        vfd.write("%s\n"%channel_name)
                    packet_list_container.append((packet_name,packet_id,packet_level,channel_list))
                    ht.num_packets += 1
        ...
        for channel in channel_size_dict:
            if not channel in packetized_channel_list and not channel in ignore_name_list:
                (channel_id,channel_size) = channel_size_dict[channel]
                print("Channel %s (%d) not packetized or ignored."%(channel,channel_id))
        ...
        levels.sort()
        for level in levels:
            print("Level: %s Bytes: %d bits: %d"%(level,size_dict[level],size_dict[level]*8))
        print("Number of packets: %d\nTotal packet bytes: %d bits: %d"%(ht.num_packets,total_packet_size,total_packet_size*8))
...
def pinit():
    ...
    parser.add_option("-d", "--dependency-file", dest="dependency_file", type="string", help="Generate dependency file for make", default=None)
